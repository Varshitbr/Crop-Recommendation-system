...
# Mapping only used if model returns numeric values
crop_dict = {
...
def predict():
    try:
        ...
        # Step 5: Handle both numeric & string model outputs safely
        if isinstance(prediction, (int, np.integer)):
            crop_name = crop_dict.get(prediction, "Unknown Crop")
        else:
            crop_name = str(prediction).capitalize()

        result = f"🌾 {crop_name} is the best crop to be cultivated in the given conditions!"
